...
public class MCP3428
{
    ...
    public void setConfig(int channel, int gain)             // Select Channel and Gain
    {
        ...
        byte reg =  (byte)((1 << BIT_RDY) | (channel << BIT_C0) | (1 << BIT_OC) | (3 << BIT_S1) | gain);

        try{

            mcp3428.write((byte)Integer.parseInt("11111011", 2));    // write configuration into config register.
        }
        catch (Exception ex)
        {
    ...
    public static void main(String[] args)
    {
        ...
        MCP3428 sensor = new MCP3428(0x6B);

        final NumberFormat NF = new DecimalFormat("##00.00");
        sensor.setConfig(CHANNEL_3,GAIN_8);     //selected Channel = 0 and Gain = 1x
